...
if (test-path("$(split-path $SCRIPT:MyInvocation.MyCommand.Path)\Kemp\Kemp.LoadBalancer.Powershell.psm1")) {
	Import-Module "$(split-path $SCRIPT:MyInvocation.MyCommand.Path)\Kemp\Kemp.LoadBalancer.Powershell.psm1" # -DisableNameChecking -Scope Global
} else {
...
if (!(Get-Module Kemp.LoadBalancer.Powershell)) {
    Set-PrtgError "Kemp.LoadBalancer.Powershell module not loaded"
}
if (!(Get-Command Send-LBMessage -errorAction SilentlyContinue)) {
    Set-PrtgError "Send-LBMessage Command not Found"
}


try {
    $response = Send-LBMessage -command stats -LoadBalancer $KEMPCluster -Credential $KempCredentials
} catch {
	Set-PrtgError "Unable to query $KEMPCluster - $_"
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQC3lz8TGxmNQbadMTxzRkZeE
# ipqgghKzMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDfDDhZ4biYjJGt99p6Ow0UhyoD4MA0G
# CSqGSIb3DQEBAQUABIIBABOxWgTcdmerrxtbaLC6LTxIGWu889U96uDYd8RvN/Gl
# d2PUfgy8vdqHHBCYstobdZvrqMkw8P/bwFkIfxFHXJbhvlAJbDevrd282JeQrN6N
# Zv+g+ef9gIp7jXRvLIup9rvTyoUApbZFizu6iPP6Cv04oPEvjFxAW7i847f4C8Ga
# pLqCP5xyBUkqZ+FBKoRMdv/bVdPUcogD7hbzX5HlrqVcJvm5s9pArSdwEpUqZiXq
# jl3CyMSe6X/pvZCEOr0+xV1e9ylTEEdHynH3r+M5GgwRYHYTkZN3YW5FYOT2vlE4
# t7ZDQ4Jn2GSolsMiDFOS3V3AaakGFVKMGG/8xtyi9XahggILMIICBwYJKoZIhvcN
...
# MjIwOTA5WjAjBgkqhkiG9w0BCQQxFgQUgfTMSLpiJP8J8fTPvBZDZU/jCAcwDQYJ
# KoZIhvcNAQEBBQAEggEAA3GbieM3KnjRSqKRi7nIAiwh+EaxzP4AaCdtnbwq6BHE
# k2dU+QQt3eRhNSbj3Hv5qR95UX1z3A7BZUWYz/HDchYoalBFUMx7Bx5WEs4XrSfg
# +vrk9Q0Kj2nh9+VR92x97zU7i4qYUJhhgr6eVc4RbiT52j/nVC6Nxc26SKnFrELh
# enldIjBKva2QiUbIewf4N+Pk0mpP7th0kmzGicuTSDhx372zf1i7ztSWhN7I67d4
# mEbMu1bHwJKH/Nt7ux5uALrfMw1lSNDh18CbDdE7iaUxfzgOqM7IzZvORWtGKX9k
# PBTd/uFKlbswExCmn2fPm5DWeq8S7TZI9kZW3b/NFA==
