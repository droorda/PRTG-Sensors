if ($null -eq (Get-Module -Name ActiveDirectory -ErrorAction SilentlyContinue)) {
    ...
    if ($null -eq (Get-Module -Name ActiveDirectory -ErrorAction SilentlyContinue)) {
        ...
        Write-Host "  <text>Unable Load ActiveDirectory Module</text>"
        Write-Host "</prtg>"
        exit
    } else {
        Write-verbose "    Success Loading ActiveDirectory Module"
    }
}


$LockedOut      =Search-ADAccount -LockedOut                   -UsersOnly | Where-Object {$_.Enabled -eq $True} | Select-Object SamAccountName
$PasswordExpired=Search-ADAccount -PasswordExpired             -UsersOnly | Where-Object {$_.Enabled -eq $True} | Select-Object SamAccountName
$AccountExpired =Search-ADAccount -AccountExpired              -UsersOnly | Where-Object {$_.Enabled -eq $True} | Select-Object SamAccountName
$AccountExpiring=Search-ADAccount -AccountExpiring -TimeSpan 7 -UsersOnly | Where-Object {$_.Enabled -eq $True} | Select-Object SamAccountName


Write-Host "<prtg>"
Write-Host "  <result>"
Write-Host "    <channel>Locked Out Users</channel>"
Write-Host "      <value>$(($LockedOut       | measure-object).count)</value>"
Write-Host "  </result>"

Write-Host "  <result>"
Write-Host "    <channel>Expired User Password</channel>"
Write-Host "      <value>$(($PasswordExpired | measure-object).count)</value>"
Write-Host "  </result>"

Write-Host "  <result>"
Write-Host "    <channel>Expired User Accounts</channel>"
Write-Host "      <value>$(($AccountExpired  | measure-object).count)</value>"
Write-Host "  </result>"

Write-Host "  <result>"
Write-Host "    <channel>Expiring User Accounts</channel>"
Write-Host "      <value>$(($AccountExpiring | measure-object).count)</value>"
Write-Host "  </result>"
if       ($LockedOut)       {
    $text = "Users Locked Out: $(            (($LockedOut       | Select-Object SamAccountName | ConvertTo-Csv -NoTypeInformation | Select-Object -skip 1 ) -join ", ").replace('"',''))"
} elseif ($PasswordExpired) {
    $text = "Users with Expired Passwords: $((($PasswordExpired | Select-Object SamAccountName | ConvertTo-Csv -NoTypeInformation | Select-Object -skip 1 ) -join ", ").replace('"',''))"
} elseif ($AccountExpired) {
    $text = "Expired User Accounts: $(       (($AccountExpired  | Select-Object SamAccountName | ConvertTo-Csv -NoTypeInformation | Select-Object -skip 1 ) -join ", ").replace('"',''))"
} elseif ($AccountExpiring) {
    $text = "User Accounts Expiring: $(      (($AccountExpiring | Select-Object SamAccountName | ConvertTo-Csv -NoTypeInformation | Select-Object -skip 1 ) -join ", ").replace('"',''))"
} else {
    $text = "OK"
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUh7S7rO0bpnXPWAAfbxxIEqC4
# o8ygghKzMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFE4YXHznj8wd4rJfXC1e2E/IVURwMA0G
# CSqGSIb3DQEBAQUABIIBAGWg4gpRRggFcyaZrF3K3bEMmaMK8UfjwyD1jkCZ9OUv
# k6Bkq53ygKbU2c6x8qTjBtuwEYF0mrpKMxlLq/cD0hR4xJviqkCRNP27CjRlwZ7H
# mXVzj73l/zOAvdNBarroRDCP+fQaJln8b5BSKRIGQDUDuqyPH6IhbYAxylq/awQK
# FFbkJLfWPiGGIlaTKgO9yV38x/jKeHYYQ3low8tfsB1gDrFzuewt6aRukcLyuueO
# pEPXaujEhY1ER1UAtTOQR1mKJ5c7Ey5EH7LchMQCozFJsFEsxhqPaTXJUuKfb+f/
# DPsXoYWxMjjBL1+UyCIsaTiOp8y/1nv2w5wsDaZMp86hggILMIICBwYJKoZIhvcN
...
# MjEzNTMyWjAjBgkqhkiG9w0BCQQxFgQU7n/N/vQvzrmEPP9rfmwfCtWGpP0wDQYJ
# KoZIhvcNAQEBBQAEggEAokCipNQUrLPPU1QW6DqyuD4ILnue94guaH1oKAogSY94
# FJcI/wvuSifpKB7oe6yMcRkOclKIFFruqEvnRDyQ4OF+7OPPPRjsgXU9vIY8Nf/J
# 7+RurxY5weI40Xjn94FwyHaM2jzd1i5/jcig73HpdRXYXnwsyjsAFwPTA/aAaN4g
# 6nuLtglMw80hRBj0SyRmQzKkdiWH/I3yTv88rx5vysMRuOxcj4PBVUT+UkrCRcVF
# D47KvGdBuU5Pxsuk1wHBgLXW8kC7W0FuNCqSPShvqdVP9lciMwuBh9Ku69rYwzxs
# pny+xPH/NS7iQBu9mSdaCOg9t3ubF0G0vWsii3z4wQ==
